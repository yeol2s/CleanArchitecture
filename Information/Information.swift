...
// MARK: 의존성이 타고 타고 흐르는 느낌(클린아키텍처의 핵심은 의존성이 고수준을 향해 흐르기만 하면 클린아키텍처라고 할 수 있다. 계층의 수는 크게 상관없음)
...
● 클린 아키텍처는 크게 4가지 레이어로 구성
 ...
 ● 버터플라이 아키텍처(클린아키텍처를 iOS에 맞게 구성한 내용)
    ...
    *간단하게 요약 정리하자면..
        Presentation Layer - 화면에 보이는 영역을 담당하는 레이어
        Domain Layer - 비즈니스 로직이 담겨있는 레이어(최대한 변경을 지양해야 하는 레이어) (Entities, UseCases, Repository Interface)
        Data Layer - Repository(구체타입)와 하나 이상의 데이터 소스를 포함한다.(API, DB 등등)
 */
